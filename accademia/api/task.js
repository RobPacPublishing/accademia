...
function buildChapterSubsectionPrompt(input, context, subsection, index, total, targetWords, previousSectionText) {
  ...
  return [
    ...
    '- Chiudi il punto con una conclusione logica interna alla sottosezione, senza usare la formula "in conclusione".',
...
async function continueOneSubsection({ input, context, subsection, system, existingText, targetWords, fastPath = false }) {
  ...
  const prompt = [
    ...
    `- Non limitarti ad aggiungere poche frasi: estendi davvero il testo fino all'intervallo ${CHAPTER_POINT_TARGET_MIN_WORDS}-${CHAPTER_POINT_MAX_WORDS} parole (minimo assoluto ${CHAPTER_POINT_MIN_WORDS}).`,
